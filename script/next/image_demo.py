...
pretrained = "/mnt/storage/user/wangxiaodong/LLaVA-NeXT/llama3-llava-next-8b"
...
url = "/mnt/storage/user/wangxiaodong/RLAIF-V/examples/test.jpeg"
image = Image.open(url)
image_tensor = process_images([image], image_processor, model.config)
image_tensor = [_image.to(dtype=torch.float16, device=device) for _image in image_tensor]

conv_template = "llava_llama_3" # Make sure you use correct chat template for different models
question = DEFAULT_IMAGE_TOKEN + "\nFirst describe the image, and then tell me the proper action the car should do?"
