...
class RLAIFVDataset(torch_data.Dataset):
    def __init__(self, data_dir: str, reference_model=None,
        ...
        if len(data_path) == 0:
            ...
            hf_data = hf_datasets.load_dataset("/home/user/wangxd/RL-V/RLAIF-V-Dataset")['train'].cast_column("image", hf_datasets.Image(decode=False))
...
class HIERARDataset(torch_data.Dataset):
    def __init__(self, data_dir: str, reference_model=None,
        ...
        if len(data_path) == 0:
            assert reference_model is not None, "`reference_model` is mandatory when logps do not exist."
            import json
            jsonl_path = "/home/user/wangxd/RL-V/hieracaps_ds_16000.jsonl"
            hf_data = []
...
class RLAIFVHIERDataset(torch_data.Dataset):
    def __init__(self, data_dir: str, reference_model=None,
        ...
        if len(data_path) == 0:
            ...
            hf_data = hf_datasets.load_dataset("/home/user/wangxd/RL-V/RLAIF-V-HIERAR-Dataset-10k")['train'].cast_column("image", hf_datasets.Image(decode=False))
...
class SEVADataset(torch_data.Dataset):
    def __init__(self, data_dir: str, reference_model=None,
        ...
        if len(data_path) == 0:
            ...
            import json
            json_paths = [
                "/home/user/wangxd/RL-V/seva_data/ocrvqa_dpo_8k_diffusion_step500.json",
                "/home/user/wangxd/RL-V/seva_data/textvqa_dpo_8k_filter_diffusion_step500.json"
            ]
            hf_data = []
            with open(json_paths[0], 'r') as f:
                data = json.load(f)
                for item in data:
                    item['image_id'] = os.path.join("/home/user/wangxd/RL-V/ocr-qa/ocr_vqa/images/", item['image_id'])
                hf_data = hf_data + data
            
            with open(json_paths[1], 'r') as f:
                data = json.load(f)
                for item in data:
                    item['image_id'] = os.path.join("/home/user/wangxd/RL-V/playground/data/eval/textvqa/train_images/", item['image_id'])
                hf_data = hf_data + data
...
class POVIDDataset(torch_data.Dataset):
    def __init__(self, data_dir: str, reference_model=None,
        ...
        if len(data_path) == 0:
            ...
            # data_dir = "/home/user/wangxd/RL-V/POVID-Dataset_logps"
            self.data = hf_datasets.load_dataset(data_dir)['train']
            print(f"column names: {self.data.column_names}")
            self.data = self.data.cast_column("image", hf_datasets.Image(decode=False))
        ...
        import json
        entail_score = []
        jsonl_path = "/home/user/wangxd/RL-V/POVID/POVID_preference_data_for_VLLMs_version_1_Entail_Fine_all_17184.jsonl"
        with open(jsonl_path, 'r', encoding='utf-8') as file:
            for line in file:
                json_obj = json.loads(line.strip())
...
class CSRDataset(torch_data.Dataset):
    def __init__(self, data_dir: str, reference_model=None,
        ...
        if len(data_path) == 0:
            ...
            # data_dir = "/home/user/wangxd/RL-V/CSR-Dataset_logps"
            self.data = hf_datasets.load_dataset(data_dir)['train']
            print(f"column names: {self.data.column_names}")
            self.data = self.data.cast_column("image", hf_datasets.Image(decode=False))
...
class RLHFDataset(torch_data.Dataset):
    def __init__(self, data_dir: str, reference_model=None,
                 tokenizer=None, image_token_len=None, img_processor=None, use_im_start_end=True, is_llava15=False):
        super().__init__()
        ...
        if len(data_path) == 0:
            ...
            hf_data = []
            jsonl_path = "/home/user/wangxd/RL-V/LLaVA-rlhf-debate/llava_rlhf_merge-for_leanpo-1w.jsonl"
            with open(jsonl_path, 'r', encoding='utf-8') as file:
                for line in file:
                    json_obj = json.loads(line.strip())
                    hf_data.append(json_obj)
            ...
            print(f'RLHF dataset length is {len(hf_data)}')
...
class EntailDataset(torch_data.Dataset):
    def __init__(self, data_dir: str, reference_model=None,
        ...
        # jsonl_path = "/home/user/wangxd/RL-V/RLAIF-V-Entail-0_10000.jsonl" # coarse
        # hard code using the fine entailment data
        jsonl_path = "/home/user/wangxd/RL-V/RLAIF-V-Entail-fine-0_10000.jsonl" # fine
        entail_score = []
        with open(jsonl_path, 'r', encoding='utf-8') as file:
            for line in file:
                json_obj = json.loads(line.strip())
