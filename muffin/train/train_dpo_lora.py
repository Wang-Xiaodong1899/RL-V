...
class LazyVHIERSupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning."""
    ...
    def __init__(self,
        ...
        rank0_print("Formatting inputs...Skip in lazy mode")
        self.tokenizer = tokenizer
        self.list_data_dict = our_data
        self.data_args = data_args

    def data_preprocess(self, our_data, our_data_path):
        our_data_dict = []
        for idx in range(len(our_data)):
            image_id = our_data[idx]["image"]["path"] # for decode image
            image = bytes_to_PIL_image(our_data[idx]['image']['bytes'])
            chosen = our_data[idx].get("chosen", "")
            rejected = our_data[idx].get("rejected", "")
            question = our_data[idx].get("question", "")
            question = "<image>\n" + question
            our_data_dict.append({
                "id": image_id,
                "image": image,
...
class LazyCSRSupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning."""
    ...
    def __init__(self,
        ...
        super(LazyCSRSupervisedDataset, self).__init__()
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    if data_args.dataset_name == "SeVa":
    ...
    elif data_args.dataset_name == "VHIER":
        train_dataset = LazyVHIERSupervisedDataset(tokenizer=tokenizer, data_args=data_args)
    elif data_args.dataset_name == "CSR":
        train_dataset = LazyCSRSupervisedDataset(tokenizer=tokenizer, data_args=data_args)
    else:
        raise ValueError(f"Unsupported dataset name: {data_args.dataset_name}")
