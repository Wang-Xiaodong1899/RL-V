...
from llava_hound_dpo.llava.constants import IGNORE_INDEX, X_TOKEN_INDEX, DEFAULT_X_TOKEN, DEFAULT_X_START_TOKEN, DEFAULT_X_END_TOKEN
from torch.utils.data import Dataset
from llava_hound_dpo.llava.train.llava_trainer import LLaVADPOTrainer
from data_processing.utils import load_jsonl, load_json

from llava_hound_dpo.llava import conversation as conversation_lib
conversation_lib.default_conversation = conversation_lib.conv_templates["vicuna_v1"]
from llava_hound_dpo.llava.model import *
from llava_hound_dpo.llava.mm_utils import tokenizer_X_token
...
class DataArguments:
    ...
    for_get_frames_num: Optional[int] = field(default=4)
...
def load_video_dir(video_path, args):
    frame_names = os.listdir(video_path)
    valid_names = []
    for name in frame_names:
        if name.endswith(".png") or name.endswith(".jpg") or name.endswith(".jpeg"):
            valid_names.append(name)
    total_frame_num = len(valid_names)
    ...
    for idx in frame_idx:
        ...
        pil_img = Image.open(os.path.join(video_path, name))
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            if self.training_modal == 'image':
                ...
                # TODO read from video dir
                video = load_video_dir(video, self.data_args)
                # print(video)
...
def train(attn_implementation):
    ...
    if model_args.overwrite == True:
        ...
        if "224" in cfg_pretrained.mm_vision_tower:
            ...
            least_token_number = model_args.for_get_frames_num*(16//model_args.mm_spatial_pool_stride)**2 + 1000
        else:
            least_token_number = model_args.for_get_frames_num*(24//model_args.mm_spatial_pool_stride)**2 + 1000
        ...
        _, model, image_processor, context_len = load_pretrained_model(model_args.model_name_or_path, None, model_name, load_8bit=model_args.load_8bit, overwrite_config=overwrite_config)
    else:
        _, model, image_processor, context_len = load_pretrained_model(model_args.model_name_or_path, None, model_name)
    ...
    if model_args.vision_tower is not None:
        ...
        data_args.video_processor = lambda x: vision_tower.image_processor(x)['pixel_values'][0]
